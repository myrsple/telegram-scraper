...
def _resource_root():
    """Get resource root for dev, py2app, or PyInstaller."""
    if getattr(sys, 'frozen', False):
        meipass = getattr(sys, '_MEIPASS', None)
        if meipass:
            return Path(meipass)
        # py2app bundles resources under .app/Contents/Resources
...
ZOCO_URL = None
DONATE_URL = None
SITE_URL = "https://zocoloco.xyz"
X_URL = "https://x.com/zoco_loco"
GITHUB_URL = "https://github.com/myrsple/telegram-scraper"
...
class TelegramScraperApp(ctk.CTk):
    ...
    def _create_gradient_backgrounds(self):
        ...
        try:
            ...
            base_path = _resource_root() / "assets"
    ...
    def _create_widgets(self):
        ...
        try:
            logo_path = _resource_root() / "assets" / "zoco.png"
        ...
        # Status indicator and header links
        header_actions = ctk.CTkFrame(header, fg_color="transparent")
        header_actions.pack(side="right", pady=8)

        self.status = StatusIndicator(header_actions)
        self.status.pack(side="left", padx=(0, 12))

        links_frame = ctk.CTkFrame(header_actions, fg_color="transparent")
        links_frame.pack(side="left")

        icon_size = (16, 16)
        assets_root = _resource_root() / "assets"

        globe_img = Image.open(assets_root / "Globe.png").convert("RGBA")
        globe_img = globe_img.resize(icon_size, Image.Resampling.LANCZOS)
        self._header_icons["globe"] = ctk.CTkImage(light_image=globe_img, dark_image=globe_img, size=icon_size)

        x_img = Image.open(assets_root / "x-logo.png").convert("RGBA")
        x_img = x_img.resize(icon_size, Image.Resampling.LANCZOS)
        self._header_icons["x"] = ctk.CTkImage(light_image=x_img, dark_image=x_img, size=icon_size)

        gh_path = assets_root / "github-icon.png"
        gh_img = Image.open(gh_path).convert("RGBA")
        gh_img = gh_img.resize(icon_size, Image.Resampling.LANCZOS)
        self._header_icons["github"] = ctk.CTkImage(light_image=gh_img, dark_image=gh_img, size=icon_size)

        globe_btn = HoverButton(
            links_frame,
            text="",
            image=self._header_icons["globe"],
            command=lambda: self._open_link(SITE_URL, "website"),
            fg_color=COLORS["bg_input"],
        ...
        globe_btn.pack(side="left", padx=(0, 8))

        x_btn = HoverButton(
            links_frame,
            text="",
            image=self._header_icons["x"],
            command=lambda: self._open_link(X_URL, "X"),
            fg_color=COLORS["bg_input"],
            hover_color=COLORS["border"],
            border_width=1,
            border_color=COLORS["border"],
        ...
        x_btn.pack(side="left", padx=(0, 8))

        github_btn = HoverButton(
            links_frame,
            text="",
            image=self._header_icons["github"],
            command=lambda: self._open_link(GITHUB_URL, "GitHub"),
            fg_color=COLORS["bg_input"],
            hover_color=COLORS["border"],
            border_width=1,
            border_color=COLORS["border"],
        ...
        try:
            mascot_path = _resource_root() / "assets" / "zoco-young.gif"
